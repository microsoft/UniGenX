...
def kbash_alignment(coor1, coor2):
    ...
    rotation = (
        U @ torch.diag(torch.Tensor([1, 1, torch.sign(torch.det(A))]).to(coor1.device))
    ) @ Vt
    translation = (
        coor2.mean(dim=0) - (rotation @ coor1.mean(dim=0, keepdim=True).t()).t()
    )
...
def sequence_alignment(seq1, seq2, return_seqid_only=False):
    ...
    alignments = pairwise2.align.globaldd(
        seq1, seq2, matlist.blosum62, -11, -1, -11, -1
    )
    ...
    for aa1, aa2 in zip(align_best[0], align_best[1]):
        if aa1 == "-":
            ...
            if aa2 == "-":  # aa1 is not gap and aa2 is
                ...
                aa1 != aa2
            ):  # and aa2 != '-', both are not gaps and aa1 is different from aa2
                idx_map_s2tos1_list.append(-1)
                idx_map_s1tos2_list.append(-1)
...
def statistical_metric(vars1, vars2, num_bin=100, plot=False):
    ...
    v_max, v_min = np.max(np.concatenate([vars1, vars2])), np.min(
        np.concatenate([vars1, vars2])
    )
