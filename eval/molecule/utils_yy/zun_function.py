...
def feats2DM(feats):
    ...
    sparse_DM = np.zeros((len(feats), len(feats)))

    sparse_DM[:3, :3] += DM_list[0][1:, 1:]
    sparse_DM[-1, -1] += DM_list[0][0, 0]
    sparse_DM[-1, :3] += DM_list[0][0, 1:]
    sparse_DM[:3, -1] += DM_list[0][1:, 0]
    for i in range(len(feats) - 3):
        sparse_DM[i : (i + 4), i : (i + 4)] += DM_list[i + 1]

    sparse_DM[-3:, -3:] += DM_list[-2][:3, :3]
    sparse_DM[0, 0] += DM_list[-2][-1, -1]
    sparse_DM[0, -3:] += DM_list[-2][-1, :3]
    sparse_DM[-3:, 0] += DM_list[-2][:3, -1]

    sparse_DM[-2:, -2:] += DM_list[-1][:2, :2]
    sparse_DM[:2, :2] += DM_list[-1][2:, 2:]
    sparse_DM[-2:, :2] += DM_list[-1][:2, 2:]
    sparse_DM[:2, -2:] += DM_list[-1][2:, :2]
    ...
    for i in range(len(feats)):
        sparse_DM[i, i] /= 4
        sparse_DM[i, (i + 1) % len(feats)] /= 3
        sparse_DM[i, i - 1] /= 3
        if len(feats) == 5:
            sparse_DM[i, (i + 2) % len(feats)] /= 3
            sparse_DM[i, i - 2] /= 3
        elif len(feats) == 6:
            sparse_DM[i, (i + 2) % len(feats)] /= 2
            sparse_DM[i, i - 2] /= 2
            sparse_DM[i, (i + 3) % len(feats)] /= 2
        else:
            sparse_DM[i, (i + 2) % len(feats)] /= 2
            sparse_DM[i, i - 2] /= 2
...
def DM2feats(DM, feats_ori):
    ...
    local_DM = np.zeros((4, 4))
    local_DM[-3:, -3:] = DM[:3, :3]
    local_DM[0, 0] = DM[-1, -1]
    local_DM[0, -3:] = DM[-1, :3]
    local_DM[-3:, 0] = DM[:3, -1]
    feats.append(local_feat(local_DM, feats_ori[0]))

    for i in range(len(DM) - 3):
        feats.append(local_feat(DM[i : (i + 4), i : (i + 4)], feats_ori[i + 1]))
    ...
    local_DM[-1, :3] = DM[0, -3:]
    local_DM[:3, -1] = DM[-3:, 0]
    feats.append(local_feat(local_DM, feats_ori[-2]))
    ...
    local_DM[2:, :2] = DM[:2, -2:]
    local_DM[:2, 2:] = DM[-2:, :2]
    feats.append(local_feat(local_DM, feats_ori[-1]))
...
# def build_local_coords(local_feats):  # local_feats: [rij1, rij2, rij3, theta_ij1j2, theta_ij1j3, theta_ij2j3]
#     coords = np.zeros((4, 3))
...
#         return [
#             x2 ** 2 + y2 ** 2 - rij2 ** 2,
#             (x2 - rij1) ** 2 + y2 ** 2 - rj1j2 ** 2,
#             x3 ** 2 + y3 ** 2 + z3 ** 2 - rij3 ** 2,
#             (x3 - rij1) ** 2 + y3 ** 2 + z3 ** 2 - rj1j3 ** 2,
#             (x3 - x2) ** 2 + (y3 - y2) ** 2 + z3 ** 2 - rj2j3 ** 2,
#         ]

#     res = fsolve(equation, np.random.rand(5))
#     coords[2][:2] = res[:2]
#     coords[3] = res[2:]
#     return coords
...
#     pos_N, pos0, pos1, pos2 = build_local_coords(feats[0])
#     coords = [pos0, pos1, pos2]
...
#             x, y, z = pos
#             return [
#                 (x - coords[-3][0]) ** 2 + (y - coords[-3][1]) ** 2 + (z - coords[-3][2]) ** 2 - rj1j3 ** 2,
#                 (x - coords[-2][0]) ** 2 + (y - coords[-2][1]) ** 2 + (z - coords[-2][2]) ** 2 - rij3 ** 2,
#                 (x - coords[-1][0]) ** 2 + (y - coords[-1][1]) ** 2 + (z - coords[-1][2]) ** 2 - rj2j3 ** 2,
#             ]

#         pos = fsolve(equation, np.random.rand(3))
#         coords.append(pos)

#     coords.append(pos_N)  # Use pos_N since it will determines x-axis
