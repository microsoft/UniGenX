...
def train(
    ...
    for i, data in enumerate(loader):
        ...
        atom_positions = (
            data["positions"].view(batch_size * n_nodes, -1).to(device, torch.float32)
        )
        atom_mask = (
            data["atom_mask"].view(batch_size * n_nodes, -1).to(device, torch.float32)
        )
...
def test(
    model, epoch, loader, mean, mad, property, device, log_interval, debug_break=False
):
    return train(
        model,
        epoch,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--lr", type=float, default=1e-3, metavar="N", help="learning rate"
    )
    parser.add_argument(
        "--nf", type=int, default=128, metavar="N", help="learning rate"
    )
    parser.add_argument(
        "--attention",
        type=int,
    ...
    parser.add_argument(
        "--node_attr", type=int, default=0, metavar="N", help="node_attr or not"
    )
    parser.add_argument(
        "--weight_decay", type=float, default=1e-16, metavar="N", help="weight decay"
    )
    parser.add_argument(
        "--save_path", type=float, default=1e-16, metavar="N", help="weight decay"
    )
    parser.add_argument(
        "--model_name",
        type=str,
    ...
    property_norms = utils.compute_mean_mad_from_dataloader(
        dataloaders["valid"], [args.property]
    )
    mean, mad = (
        property_norms[args.property]["mean"],
        property_norms[args.property]["mad"],
    )
        ...
        if epoch % args.test_interval == 0:
            ...
            if val_loss < res["best_val"]:
                ...
                if args.save_model:
                    torch.save(
                        model.state_dict(),
                        args.outf + "/" + args.exp_name + "/best_checkpoint.npy",
                    )
                    with open(
                        args.outf + "/" + args.exp_name + "/args.pickle", "wb"
                    ) as f:
                        pickle.dump(args, f)
            print(
